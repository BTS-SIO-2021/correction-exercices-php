...
<?php
   ...
   //2eme soluc : $prix_ttc = $prix_ht*(1+$tva/100);
...
if($sexe == 'homme'){
   ...
   echo 'Bonjour personne non identifiée';
}

//autre solution, on utilise le switch : https://www.php.net/manual/fr/control-structures.switch.php
...
if($budget >= $achats){
   echo 'Le budget '.$budget.'€ permet de payer tous les achats d\'un montant de '.$achats.'€.';
} else {
   echo 'Le budget '.$budget.'€ ne permet pas de payer tous les achats d\'un montant de '.$achats.'€.';
}
...
/* 
Je peux utiliser pour mieux contrôler la valeur de ma variable une fonction comme settype https://www.php.net/manual/fr/function.settype.php. Ca me permet d'être sûre du type de sortie de ma variable. 
*/
...
echo ($heure >=22 || $heure < 7 ? "c'est la nuit": (($heure < 12)? "c'est le matin": "c'est l'aprem"));
...
// autre solution avec tous les étapes sur une seule ligne
for($i=1; $i<=10; $i++){
   echo $m.' x '.$i.' = '.($m*$i). '<br/>';
}
...
// Ici je crée un test en instanciant une variable $test qui contient la valeur du retour de la fonction rand. Cette fonction rand doit me retourner un entier compris entre 0 et 50 (les deux paramètres que je lui ai transmis).
$test = rand(0, 50);
var_dump($test);

// Ici je crée mon tableau = j'instancie ma variable $tableau dont la valeur est égale à un tableau (pour l'instant ce tableau vide);
$tableau = []; 
var_dump($tableau);
...
while($i < 10){
   // Ici je rajoute dans mon tableau la valeur que me retourne la fonction rand(). Cette manière de rajouter une valeur dans un tableau est équivalente à array_push : https://www.php.net/manual/fr/function.array-push.php
   /*
   $valeurAleatoire = rand(0, 50);
   array_push($tableau, $valeurAleatoire);
   */
...
// Après des recherches je vois que la fonction in_array (https://www.php.net/manual/fr/function.in-array.php) me permet de vérifier si une valeur se trouve dans mon tableau, c'est exactement ce que je veux faire !
...
echo $verif== true?"Le nombre 42 est bien dans le tableau" : "Le nombre 42 n'est pas dans le tableau";
